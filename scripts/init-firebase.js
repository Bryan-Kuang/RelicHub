// This script initializes the Firebase database with a default superadmin user
// Run this script after setting up your Firebase project and updating .env.local
...
async function addSuperAdmin(phoneNumber) {
  try {
    ...
    console.log("Superadmin added with ID: ", docRef.id);
    return docRef.id;
  } catch (error) {
...
async function main() {
  ...
  if (!phoneNumber) {
    console.error(
      "Please provide a phone number in E.164 format (e.g., +1234567890)"
    );
    process.exit(1);
  }
  ...
  try {
    // Add superadmin
    await addSuperAdmin(phoneNumber);
